...
function drawGraph (data) {
  var datedelta = moment(data.data[data.data.length - 1].datetime) - moment(data.datetime)
  if (datedelta < 60 * 60 * 24 * 30 * 1000) {
    data.data.push({view: 0, danmaku: 0, like: 0, dislike: 0, share: 0, favorite: 0, coin: 0, datetime: moment(data.datetime).utcOffset(0).format('YYYY-MM-DD HH:mm')})
  }
  let graph = {
    title: {
      left: 'center',
